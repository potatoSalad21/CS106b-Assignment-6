...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
	...
	foreach(ext_char ch in frequencies) {
		...
		n->one = nullptr;
		n->zero = nullptr;
		n->weight = frequencies[ch];
		pq.enqueue(n, frequencies[ch]);
	}
...
void freeTree(Node* root) {
	...
	while (!qq.empty()) {
		Node* n = qq.front(); qq.pop();
		if (n->one) qq.push(n->one);
		if (n->zero) qq.push(n->zero);

		delete n;
	}
}

// helper function - converts encoding tree into a map
void fillMapRec(Map<ext_char, string>& encMap, string curr, Node* encTree) {
	if (!encTree) return;
	if (!encTree->zero && !encTree->one) {
		encMap[encTree->character] = curr;
		return;
	}

	fillMapRec(encMap, curr + '0', encTree->zero);
	fillMapRec(encMap, curr + '1', encTree->one);
}
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
	Map<ext_char, string> encMap;
	fillMapRec(encMap, "", encodingTree);

	char ch;
	while (infile.get(ch)) {
		string bitsStr = encMap[ch];
		for (char bit : bitsStr)
			outfile.writeBit(bit != '0');
	}

	string endOfFile = encMap[PSEUDO_EOF];
	for (char ch : endOfFile)
...
void decodeFile(ibstream& infile, Node* encodingTree, ostream& file) {
	...
	while (true) {
		...
		if (b) curr = curr->one;
		else if (!b) curr = curr->zero;

		if (curr->character == NOT_A_CHAR) continue;
